"""
TAKEN FROM: https://gist.github.com/tomalf2/d24db49f575c397e79b5872b74739542
"""
...
class VariantCaller:
    ...
    def call_variants(self, name: str, aligned: str, aggregate_close_changes: bool, one_based: bool,
                      strip_edge_insertions: bool = False) -> AlignmentChanges:
        """
        ...
        if aggregate_close_changes:
            ...
            sub_changes_pos, sub_changes_refs, sub_changes_alts = VariantCaller._aggregate_subs(sub_changes_pos,
                                                                                                sub_changes_refs,
                                                                                                sub_changes_alts)
        ...
        if name and self.insertions_df is not None:
            try:
                ...
                if strip_edge_insertions:
                    ins_changes_pos = ins_changes_pos[
                        (ins_changes_pos > 0) & (ins_changes_pos < self.refseq_len)]  # drop insertions at edges
    ...
    def _aggregate_subs(changes_pos, changes_refs, changes_alts=None) -> Tuple[np.array]:
        ...
        while (idx < num_changes):
            ...
            if next_idx < num_changes:  # check for contiguous dels only if there's another del after this
                ...
                while (next_idx < num_changes and (
                        pos + 1) == next_pos):  # loop until next del coord. == this del coord +1
                    pos = next_pos
                    next_idx += 1
                    try:
        ...
        with open(file_path, "r") as ins_file:
            ...
            for line in ins_file:
                split_columns = line.rstrip("\n").rsplit(",", maxsplit=len(
                    ins_coordinates))  # such weird logic is necessary because "strain" name can contain ","
