...
get_markers_by_human_percentage = \
    ("WITH IsolatesFiltered AS ("
     ...
     "  segment.segment_id, "
     "  CASE "
     "    WHEN isolate.host_id = -1 THEN 'Other hosts' "
     "    WHEN host.host_name = 'Human' THEN NULL "
     "    ELSE host.host_name "
     "  END AS host_name "
     "FROM Segment segment "
     "JOIN IsolatesFiltered isolate ON segment.isolate_epi = isolate.isolate_epi "
     "JOIN Subtype subtype ON isolate.subtype_id = subtype.subtype_id "
     "LEFT JOIN Host host ON host.host_id = isolate.host_id "
     "WHERE (segment.segment_type == :segment_type OR :segment_type IS NULL) "
     "AND (subtype.name == :subtype OR :subtype IS NULL) "
     "AND (isolate.host_id = -1 OR host.host_name != 'Human') "
     "), "
     ""
    "HumanMarkerCount AS ("
     ...
     "COALESCE(selectedMarkers.human_marker_count, 0) as '# Human Segments with Marker', "
     "selectedMarkers.total_marker_count AS '# Segments with Marker', "
     "selectedMarkers.percentage AS Percentage, "
     "host_info AS '# Other Hosts Segments with Marker' "
...
get_marker_host_distribution = \
    ("WITH IsolatesFiltered AS ("
     ...
     ""
     "SELECT SBH.host_name as 'Host', COUNT(DISTINCT segmentMarkers.segment_id) AS '# Segments with marker' "
     "FROM SegmentMarkers segmentMarkers "
     "JOIN SegmentsByHost SBH ON segmentMarkers.segment_id = SBH.segment_id "
...
get_most_common_markers_by_filters = \
    ("WITH RECURSIVE Descendants(host_id, parent_id, root_parent_id) AS ( "
     ...
     "JOIN Host host ON d.root_parent_id = host.host_id "
     "WHERE (host.host_name = :host OR :host IS NULL) "
     "AND (segment.segment_type = :segment_type OR :segment_type IS NULL) "
     "AND (subtype.name = :subtype OR :subtype IS NULL)), "
     ""
...
get_markers_by_relevance = (
    ...
    ""
    "MarkerCount AS ( "
    "    SELECT marker.marker_id, Segment.segment_type, COUNT(DISTINCT SegmentMarkers.segment_id) AS marker_count "
    "    FROM SegmentMarkers "
    ...
    "    JOIN Marker marker ON marker.marker_id = SegmentMarkers.marker_id "
    "    GROUP BY marker.marker_id, Segment.segment_type "
    "), "
    ""
    "TotalCount AS ("
    "    SELECT Segment.segment_type, COUNT(DISTINCT Segment.segment_id) AS total_count "
    "    FROM Segment "
    "    JOIN IsolatesFiltered ON IsolatesFiltered.isolate_epi = Segment.isolate_epi "
    "    GROUP BY Segment.segment_type "
    ") "
    ""
    "SELECT marker.name AS 'Marker', "
    "       ROUND((markerCount.marker_count * 100.0) / totalCount.total_count, 2) AS 'Percentage', "
    "       markerCount.marker_count AS 'Marker Count', "
    "       totalCount.total_count AS 'Segment Count', "
    "       markerCount.segment_type AS 'Segment Type' "
    "FROM Marker marker "
    "JOIN MarkerCount markerCount ON marker.marker_id = markerCount.marker_id "
    "JOIN TotalCount totalCount ON markerCount.segment_type = totalCount.segment_type "
...
get_segment_mutability_zones = \
    ...
    "    FROM SelectedSegments"
    "), "
    "CountPerBin AS ( "
    "    SELECT "
    "        bin.start_range, "
    "        bin.end_range, "
    "        COUNT(DISTINCT mutation.mutation_id) AS bin_count, "
    "        COUNT(*) AS mut_count "
    ...
    "    mut_count * 1.0 / TS.total_segments AS 'Ratio of mutations on segments',"
...
get_mutability_peak_months = \
 ("WITH IsolatesFiltered AS ("
  ...
  "COUNT(DISTINCT SSPM.segment_id) AS '# Segments', "
  "ROUND((COUNT(DISTINCT mutation.mutation_id) * 1.0 / COUNT(DISTINCT SSPM.segment_id)), 2) AS '#Mutation per Segment' "
...
get_markers_over_time = \
    ("WITH IsolatesFiltered AS ("
     ...
     "END "
     "BETWEEN (:global_start_year * 100 + :global_start_month) AND (:global_end_year * 100 + :global_end_month) "
     "AND length(isolate.collection_date) >= 7), "  # Add check here for safety
     ""
     "SelectedMarkers AS ( "
     "SELECT name, marker_id FROM Marker "
     "WHERE name IN (markers_placeholder)), "
     ""
     "MarkerOverTimeInformation AS ( "
     "SELECT SM.marker_id, name, COUNT(*) AS count, "
     "strftime('%Y', I.collection_date) AS year "
     "FROM SelectedMarkers SM "
     "JOIN SegmentMarkers SegM ON SM.marker_id = SegM.marker_id "
     "JOIN Segment S ON SegM.segment_id = S.segment_id "
     "JOIN IsolatesFiltered I ON S.isolate_epi = I.isolate_epi "
     "GROUP BY SM.marker_id, name, year "
     "), "
     ""
     "TotalOverTime AS ( "
     "SELECT COUNT(*) AS total_for_year, "
     "strftime('%Y', I.collection_date) AS year "
     "FROM SegmentMarkers SegM "
     "JOIN Segment S ON SegM.segment_id = S.segment_id "
     "JOIN IsolatesFiltered I ON S.isolate_epi = I.isolate_epi "
     "GROUP BY year "
     ") "
     ""
     "SELECT name AS 'Marker', MOTI.year AS 'Year', count AS '# per Year', total_for_year AS '# Total Segments per Year',"
     "count * 1.0 / total_for_year AS Percentage "
     "FROM MarkerOverTimeInformation MOTI "
     "JOIN TotalOverTime TOT ON MOTI.year = TOT.year "
     "WHERE CAST(MOTI.year AS INTEGER) > 1900")
