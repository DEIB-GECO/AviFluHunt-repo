...
class MutationDatabaseHandler:
    ...
    def execute_data_flow(self):
        ...
        for key in self.dna_fasta_by_ref.keys():
            ...
            serotype_name, segment_type = key.split('_', 1)

            references = self.get_references(serotype_name, segment_type)
                ...
                for annotation in annotations:
                    ...
                    for aligned_protein_header, aligned_protein_fasta in aligned_protein_fastas.items():
                        ...
                        mutations = self.get_mutations(aligned_protein_header, aligned_protein_fasta,
                                                       reference_coding_sequence, protein_insertions_file)
                        #self.create_segment_data(segment_id, ) TODO?

                        for mutation in mutations:
                            mut_id = self.create_mutation(header_dict['segment_serotype'], annotation, mutation)
                            self.create_segment_mutation(segment_id, reference["reference_id"], mut_id)
    ...
    def create_isolate(self, isolate_epi):
        ...
        serotype_id = self.get_serotype(isolate_metadata[12].split("/")[-1].strip())["serotype_id"]

        self.database_handler.insert_row("Isolate",
                                         ["isolate_id", "isolate_epi", "serotype_id",
                                          "host", "collection_date", "location_id"],
                                         (None, isolate_epi, serotype_id, host, collection_date, location_id,))
        return 0
    ...
    def create_segment_data(self, segment_id, sequence):
        self.database_handler.insert_row("SegmentData",
                                         ["segment_id", "sequence"],
                                         (segment_id, sequence,))

    def create_mutation(self, serotype_name, annotation, mutation):
        ...
        mutation_name = f"{serotype_name}:{annotation["annotation_name"]}:{ref}{position}{alt}"

        mutations_in_db = self.database_handler.get_rows("Mutation", ["mutation_name"], (mutation_name,))
        if mutations_in_db:
            return mutations_in_db[0]["mutation_id"]

        return self.database_handler.insert_row("Mutation",
                                                ["mutation_id", "mutation_name", "serotype_name", "annotation_id",
                                                 "annotation_name", "position", "ref", "alt"],
                                                (None, mutation_name, serotype_name, annotation["annotation_id"],
    ...
    def get_serotype(self, serotype_name):
        serotypes = self.database_handler.get_rows("Serotype", ["name"], (serotype_name,))
        if serotypes:
            return serotypes[0]
        return None

    def get_references(self, serotype_name, segment_type):

        references = []
        serotype = self.get_serotype(serotype_name)
        if serotype:
            ...
            for reference in references_of_segment:
                if self.database_handler.row_exists("ReferenceOfSerotype",
                                                    ["reference_id", "serotype_id"],
                                                    (reference["reference_id"], serotype["serotype_id"],)):
